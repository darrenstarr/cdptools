...
/** Sequential file system handler entry points for the /proc/net/cdp/summary file */
static const struct seq_operations cdp_seq_summary_ops = {
	.start  = cdp_seq_start,
	.next   = cdp_seq_next,
	.stop   = cdp_seq_stop,
	.show   = cdp_seq_summary_show,
};

/** The inode open handler for the /proc/net/cdp/summary file
  ...
  */
static int cdp_seq_summary_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &cdp_seq_summary_ops);
}

/** The proc_fs inode entry points for processing the /proc/net/cdp/summary file */
static const struct file_operations cdp_seq_summary_fops = {
	.open		= cdp_seq_summary_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= seq_release,
};

/** Sequential file system handler entry points for the /proc/net/cdp/detail file */
static const struct seq_operations cdp_seq_detail_ops = {
	.start  = cdp_seq_start,
	.next   = cdp_seq_next,
	.stop   = cdp_seq_stop,
	.show   = cdp_seq_detail_show,
};

/** The inode open handler for the /proc/net/cdp/detail file
  *  @param inode the inode structure to provide entry points for.
  *  @param file the file to provide entry points for processing to.
  *  @return 0 on success, negative values on failure.
  */
static int cdp_seq_detail_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &cdp_seq_detail_ops);
}

/** The proc_fs inode entry points for processing the /proc/net/cdp/detail file */
static const struct file_operations cdp_seq_detail_fops = {
	.open		= cdp_seq_detail_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= seq_release,
};
...
int __init cdp_proc_init(void)
{
    ...
    cdp_summary_proc_entry = proc_create("summary", 0444, cdp_proc_dir, &cdp_seq_summary_fops);
    ...
    cdp_detail_proc_entry = proc_create("detail", 0444, cdp_proc_dir, &cdp_seq_detail_fops);
